...
namespace ioctlpus
{
    public partial class MainForm : Form
    {
        ...
        private void hbInputTb_TextChanged(object sender, EventArgs e)
        {
            if(this.hbInputCb.Checked)
            {
                //宽字节
                string text = this.hbInputTb.Text;
                byte[] wideBytes = Encoding.Unicode.GetBytes(text);
                DynamicByteProvider requestData = new DynamicByteProvider(wideBytes);
                hbInput.ByteProvider = requestData;
                this.nudInputSize.Value = wideBytes.Length;
            }
            else
            {
                //assic
                string text = this.hbInputTb.Text;
                byte[] assicBytes = Encoding.ASCII.GetBytes(text);
                DynamicByteProvider requestData = new DynamicByteProvider(assicBytes);
                hbInput.ByteProvider = requestData;
                this.nudInputSize.Value = assicBytes.Length;
            }
            this.hbInput.Invalidate();
        }
    }
